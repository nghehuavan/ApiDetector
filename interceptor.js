    ...
    responseClone
      ...
      .then((responseBody) => {
        if (isActive) {
          ...
          // Check if the URL contains .ashx or if the content type is JSON
          const uri = new URL(url.toString());
          const isAshx = uri.pathname.toLowerCase().endsWith('.ashx');
  ...
  window.XMLHttpRequest.prototype.send = function (body) {
    ...
    this.onload = function () {
      ...
      if (isActive) {
        ...
        // Check if the URL contains .ashx or if the content type is JSON
        const isAshx = this._requestUrl.toString().toLowerCase().includes('.ashx');
        const isJson = contentType && contentType.includes('application/json');
