...
// Main execution flow
(async function () {
  // Always inject the interceptor script and wait for it to load
  await injectInterceptor();

  // After injecting and loading, send the initial activation state to the interceptor
  chrome.storage.local.get('enabledDomains', ({ enabledDomains }) => {
    const domains = enabledDomains || [];
    const isActive = domains.includes(window.location.hostname);
